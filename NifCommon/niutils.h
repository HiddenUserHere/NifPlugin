...
inline T GetIniValue(LPCWSTR Section, LPCWSTR Setting, T Default, LPCWSTR iniFileName) {
	...
	sstr << Default;
	buffer[0] = 0;
	if (0 < MaxSDK::Util::GetPrivateProfileStringW(Section, Setting, sstr.str().c_str(), buffer, sizeof(buffer), iniFileName)) {
...
inline std::wstring GetIniValue<std::wstring>(LPCWSTR Section, LPCWSTR Setting, std::wstring Default, LPCWSTR iniFileName) {
	wchar_t buffer[1024];
	buffer[0] = 0;
	if (0 < MaxSDK::Util::GetPrivateProfileStringW(Section, Setting, Default.c_str(), buffer, sizeof(buffer), iniFileName)) {
...
inline TSTR GetIniValue<TSTR>(LPCWSTR Section, LPCWSTR Setting, TSTR Default, LPCWSTR iniFileName){
   wchar_t buffer[1024];
   buffer[0] = 0;
   if (0 < MaxSDK::Util::GetPrivateProfileStringW(Section, Setting, Default.data(), buffer, sizeof(buffer), iniFileName)){
      return TSTR(buffer);
   }
   return Default;
}
...
inline void SetIniValue(LPCSTR Section, LPCSTR Setting, T value, LPCSTR iniFileName){
   stringstream sstr;
   sstr << value;
   MaxSDK::Util::WritePrivateProfileStringA(Section, Setting, sstr.str().c_str(), iniFileName);
}
template<typename T>
inline void SetIniValue(LPCWSTR Section, LPCWSTR Setting, T value, LPCWSTR iniFileName) {
	wstringstream sstr;
	sstr << value;
    MaxSDK::Util::WritePrivateProfileStringW(Section, Setting, sstr.str().c_str(), iniFileName);
}
