	...
	case WM_INITDIALOG: 
		{
			...
			so->GetParamBlock(0)->GetValue( PB_MATERIAL, 0, sel, valid);
	...
	case WM_COMMAND:
		...
		case IDC_CB_MATERIAL:
			if (HIWORD(wParam)==CBN_SELCHANGE) {
				so->GetParamBlock( 0 )->SetValue( PB_MATERIAL, 0, mCbMaterial.selection() - 1 );
			}
			break;
		}
		break;	
   ...
   if (msg == MOUSE_FREEMOVE)
   {
      ...
      switch(point) {
         case 0:
            ...
            sp0 = m;
            ob->GetParamBlock( 0 )->SetValue(PB_WIDTH,0,0.0f);
            ob->GetParamBlock( 0 )->SetValue(PB_LENGTH,0,0.0f);
            ob->GetParamBlock( 0 )->SetValue(PB_HEIGHT,0,0.0f);
            ob->GetParamBlock( 0 )->SetValue(PB_SCALE,0,NifPropsGlobals::bhkScaleFactor);
            ob->suspendSnap = TRUE;								
         ...
         case 1:
            ...
            ob->GetParamBlock( 0 )->SetValue(PB_WIDTH,0,float(fabs(d.x / s / 2.0f)));
            ob->GetParamBlock( 0 )->SetValue(PB_LENGTH,0,float(fabs(d.y / s / 2.0f)));
            ob->GetParamBlock( 0 )->SetValue(PB_HEIGHT,0,float(fabs(d.z / s / 2.0f)));
