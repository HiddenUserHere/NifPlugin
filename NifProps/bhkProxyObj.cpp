...
BOOL ProxyPickObjectMode::Filter(INode *node)
{
	if (node) {
		...
		for (int i = 0;i < mod->GetParamBlock( 0 )->Count(PB_MESHLIST); i++) {
			INode *tnode = NULL;
			mod->GetParamBlock( 0 )->GetValue(PB_MESHLIST,0,tnode,FOREVER,i);
			if  (node == tnode)
				return FALSE;
		}
...
BOOL ProxyPickObjectMode::Pick(IObjParam *ip,ViewExp *vpt)
{
	...
	if (INode *node = vpt->GetClosestHit()) {
		...
		if (os.obj->CanConvertToType(triObjectClassID)) {
			mod->GetParamBlock( 0 )->Append(PB_MESHLIST,1,&node,1);
			rv = TRUE;
		}
...
namespace
{
	...
	INT_PTR SubShapeDlgProc::DlgProc (TimeValue t,IParamMap2 *map,HWND hWnd,UINT msg,WPARAM wParam,LPARAM lParam)
	{
		...
		case WM_INITDIALOG:
			{
				...
				mod->GetParamBlock( 0 )->GetValue( PB_LAYER, INFINITE, sel, valid);
				mCbLayer.select( sel );
		...
		case WM_COMMAND:
			...
			case IDC_CB_LAYER:
				if (HIWORD(wParam)==CBN_SELCHANGE) {
					mod->GetParamBlock( 0 )->SetValue( PB_LAYER, INFINITE, mCbLayer.selection() );
				}
				break;
