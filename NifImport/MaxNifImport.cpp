...
{
    extern void ClearTexturesCacheNIF();
    ClearTexturesCacheNIF();

    extern int NUM_BIPED_NUMERED;
    NUM_BIPED_NUMERED = 50;

    extern std::vector<tstring> vObjNeedSkin;
    vObjNeedSkin.clear();

    extern std::vector<tstring> vObjNeedDelete;
    vObjNeedDelete.clear();
   ...
   if ( ok )
   {
       ...
       if ( vObjNeedSkin.size() > 0 )
       {
           strText += TEXT( "The following objects may need to be skinned:\n" );

           for ( auto str : vObjNeedSkin )
           {
               strText += str;
               strText += TEXT( "\n" );
           }
       ...
       if ( vObjNeedDelete.size() > 0 )
       {
           strText += TEXT( "The following objects must be deleted:\n" );

           for ( auto str : vObjNeedDelete )
           {
               strText += str;
               strText += TEXT( "\n" );
           }
       ...
       if ( strText.length() > 0 )
           MessageBoxW( 0, strText.c_str(), TEXT( "Warning!" ), MB_OK | MB_ICONWARNING );
   }
