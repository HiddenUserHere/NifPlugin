...
int TEXTURE_NUM_WHEEL = 1;


struct TextureSaveCacheNIF
{
	tstring strTextureOld;
	tstring strTextureNew;
};

std::vector<TextureSaveCacheNIF> vTexturesNIF;

void ClearTexturesCacheNIF()
{
	vTexturesNIF.clear();
...
{
	auto DoImportTexture = [this, desc]( tstring filename, NiSourceTextureRef texSrc ) -> Texmap *
	{
	...
	if (NiSourceTextureRef texSrc = desc.source) {
		...
		if (bmpMgr->CanImport(filename.c_str())) {
			return DoImportTexture( filename, texSrc );
		}
		//Nif
		else
		{
			if ( (filename.find( TEXT( "_r.nif" ) ) != tstring::npos) || (filename.find( TEXT( "_g.nif" ) ) != tstring::npos) || (filename.find( TEXT( "_b.nif" ) ) != tstring::npos) )
				return nullptr;

			tstring texFileName = this->path;
			texFileName += filename;

			LPCTSTR ext = PathFindExtension( texFileName.c_str() );

			if ( _tcsicmp( ext, TEXT( ".NIF" ) ) == 0 )
			{
				for ( auto & s : vTexturesNIF )
				{
					if ( s.strTextureOld == texFileName )
						return DoImportTexture( s.strTextureNew, texSrc );
				}

				TextureSaveCacheNIF sSaveTexture;
				sSaveTexture.strTextureOld = texFileName;

				NifImporter importer( texFileName.c_str(), i, gi, TRUE );
				if ( !importer.isValid() )
					return FALSE;
				if ( importer.DoImport() && importer.blocks.size() == 1 )
				{
					auto pPixels = SelectFirstObjectOfType<NiPixelData>( importer.blocks );

					texFileName = TEXT( "C:\\textures\\" );
					CreateDirectoryW( texFileName.c_str(), NULL );
					texFileName += PathFindFileName( this->name.c_str() );
					texFileName.erase( texFileName.end() - 4, texFileName.end() );

					CreateDirectoryW( texFileName.c_str(), NULL );

					texFileName += TEXT( "\\tex" );
					texFileName += std::to_wstring( TEXTURE_NUM_WHEEL++ );

					texFileName += TEXT( ".png" );

					//creating BitmapInfo 
					BitmapInfo bi;
					bi.SetType( BMM_TRUE_32 );
					bi.SetFlags( MAP_HAS_ALPHA );
					bi.SetWidth( pPixels->GetWidth() );
					bi.SetHeight( pPixels->GetHeight() );
					bi.SetCustomFlag( 0 );
					bi.SetPath( MaxSDK::Util::Path( texFileName.c_str() ) );

					Bitmap * pBitmap;
					pBitmap = bmpMgr->Create( &bi );

					auto vPixels = pPixels->GetColors();
					...
					for ( int i = 0; i < pBitmap->Height(); i++ )
					{
						...
						{
							BMM_Color_fl pixelscheck;

							pixelscheck.r = vPixels[k].r;
							pixelscheck.g = vPixels[k].g;
							pixelscheck.b = vPixels[k].b;
							pixelscheck.a = vPixels[k].a;

							k++;

							//fill pixelscheck for each row 
							pBitmap->PutPixels( j, i, 1, &pixelscheck );
						}
					}

					pBitmap->OpenOutput( &bi );
					pBitmap->Write( &bi, BMM_SINGLEFRAME );
					pBitmap->Close( &bi );
				}

				sSaveTexture.strTextureNew = texFileName;
				vTexturesNIF.push_back( sSaveTexture );

				return DoImportTexture( texFileName, texSrc );
			}
		}
	}
	return nullptr;
