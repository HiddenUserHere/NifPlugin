...
int NUM_BIPED_NUMERED = 50;

std::vector<tstring> vObjNeedSkin;
std::vector<tstring> vObjNeedDelete;

bool IsInsideBiped( INode * pNode )
{
	if ( pNode )
	{
		if ( pNode->GetParentNode() )
		{
			if ( _tcsicmp( pNode->GetParentNode()->GetName(), TEXT( "Bip01" ) ) == 0 )
				return true;

			return IsInsideBiped( pNode->GetParentNode() );
		}
	}
...
INode * GetNodeByName( tstring str )
{
	return GetCOREInterface()->GetINodeByName( str.c_str() );
}

void AddParentBoneAsSkinBone( INode * pNode )
{
	if ( pNode && pNode->GetParentNode() )
	{
		...
		Modifier * mod = (Modifier *)GetCOREInterface()->CreateInstance( OSM_CLASS_ID, SKIN_CLASSID );
		...
		vector<tstring> bonesNames;
		bonesNames.push_back( pNode->GetParentNode()->GetName() );

		for ( unsigned int j = 0; j < bonesNames.size(); j++ )
		{
			int numVertex = j;
			tstring boneName = bonesNames[j];

			//Now we can get the bone node since we already gone through each geomobject..
			INode * pBoneNode = GetNodeByName( boneName.c_str() );
			if ( pBoneNode )
			{
				bool exists = false;
...
void RenameBonesChilds( INode * pNode )
{
	if ( (pNode->GetMtl() == nullptr) || pNode->IsHidden() )
	{
		if ( tstring( pNode->GetName() ).find( TEXT( "Bip01" ) ) == tstring::npos )
		{
			tstring strNewName = TEXT( "Bip01 " ) + std::to_wstring( NUM_BIPED_NUMERED++ );
			pNode->SetName( strNewName.c_str() );

			if ( pNode->NumberOfChildren() > 0 )
			{
				for ( int i = 0; i < pNode->NumberOfChildren(); i++ )
					RenameBonesChilds( pNode->GetChildNode( i ) );
			}
		}
	}
	else
	{
		if ( IsInsideBiped( pNode ) )
		{
			tstring strText = TEXT( "[" ) + tstring( pNode->GetName() ) + TEXT( "] needs the bone [" ) + tstring( pNode->GetParentNode()->GetName() ) + TEXT( "]!" );

			AddParentBoneAsSkinBone( pNode );

			pNode->Detach( 0 );

//			vObjNeedSkin.push_back( strText );
		}
	}
}

void deleteAllChildren( INode * node )
{
	if ( node == NULL )
		return;
	
	if ( HelperObject * switchObj = (HelperObject *)GetCOREInterface()->CreateInstance( HELPER_CLASS_ID, Class_ID( BONE_CLASS_ID, 0 ) ) )
	{
		node->SetObjectRef( switchObj );
		node->ShowBone( 0 );
...
bool NifImporter::DoImport()
{
	...
	if (isValid()) {
		...
		if (root->IsDerivedType(NiNode::TYPE))
		{
			...
			{
				vector<string> importedNodes = GetNamesOfNodes( nodes );
				for ( vector<string>::iterator itr = importedNodes.begin(); itr != importedNodes.end(); ++itr )
				{
					if ( INode * node = gi->GetINodeByName( A2TString( *itr ).c_str() ) )
					{
						if ( _tcsicmp( node->GetName(), TEXT( "bound" ) ) == 0 )
						{
